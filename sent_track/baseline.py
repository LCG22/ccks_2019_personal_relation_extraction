...
class Baseline:
    ...
    def load_wordMap(self):
        ...
        wordMap['UNK'] = len(wordMap)
        all_content = []
        for line in open(os.path.join(self.data_path, 'sent_train.txt'), encoding="utf-8"):
    ...
    def run_test(self, sess, test_batchers):
        ...
        if self.bag:
            with open('result_bag.txt', 'w', encoding="utf-8") as fw:
            ...
            with open('result_sent.txt', 'w', encoding="utf-8") as fw:
                for i in range(self.data_size):
                    rel_one_hot = [int(num) for num in all_preds[i].tolist()]
                    rel_list = []
                    for j in range(0, self.num_classes):
